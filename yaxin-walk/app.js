...
const scene=new THREE.Scene();scene.background=new THREE.Color('#e9eff3');scene.fog=new THREE.Fog('#e9eff3',150,360);
const camera=new THREE.PerspectiveCamera(48,innerWidth/innerHeight,.06,450);
const target=new THREE.Vector3(28.4,1.8,-9.15);
const layers={};const keys=new Set();const input={x:0,y:0};
let orbit,lookPointer=null,joyPointer=null,lastLook={x:0,y:0};
const points=[{"id":"entrance","title":"车间主入口","text":"从卷帘门进入培训车间，沿参观通道可到达理论教室、钳工区与纺织设备区。","anchor":[3.95,3.5,-18.2],"pos":[4.1,1.65,-16.6],"look":[12,1.65,-16.2]},{"id":"room","title":"理论培训教室","text":"配有培训桌椅、显示设备、白板与讲台，用于理论教学和安全培训。","anchor":[5.5,3.1,-14.7],"pos":[6.15,1.65,-12.9],"look":[4.2,1.65,-3]},{"id":"fitter","title":"钳工实训区","text":"沿用模型中的工作台、台虎钳及车钻磨设备，查看实训空间与操作位置。","anchor":[11.9,3.1,-14.7],"pos":[11.9,1.65,-13.2],"look":[12.5,1.65,-5]},{"id":"winding","title":"自动络筒机","text":"将纱线整理成卷装，可沿通道观察机台布置与墙面培训展牌。","anchor":[20,2.8,-13.2],"pos":[19.8,1.65,-16],"look":[20,1.5,-10]},{"id":"spinning","title":"细纱机","text":"细纱设备区，用于认识牵伸、加捻和卷绕相关结构。","anchor":[23.8,2.8,-12.5],"pos":[24,1.65,-16],"look":[23.8,1.5,-10]},{"id":"roving","title":"粗纱机","text":"保留原模型的主机及后部支架，可从主通道观察设备全长。","anchor":[31.4,3.4,-12.5],"pos":[31.8,1.65,-16],"look":[31,1.8,-10]},{"id":"combing","title":"精梳机","text":"精梳设备区，沿用原工程的设备、名称牌和培训说明。","anchor":[36.2,2.5,-12.8],"pos":[36.3,1.65,-16],"look":[36,1.4,-10]},{"id":"drawing","title":"并条机","text":"并条设备区，用于观察设备空间与工序衔接。","anchor":[42.1,2.6,-12.5],"pos":[42.1,1.65,-16],"look":[42.1,1.4,-11]},{"id":"carding","title":"梳棉设备区","text":"车间内有两台梳棉设备，分别保留在原模型的两个位置。","anchor":[52,2.8,-12.5],"pos":[48,1.65,-16.4],"look":[52,1.5,-11]}];
...
function overviewPosition(){return innerWidth<600?new THREE.Vector3(104,78,113):new THREE.Vector3(74,46,55);}
...
function setMode(mode){
 ...
 if(mode==='top'){setLayer('roof',false);setCamera(new THREE.Vector3(28.4,innerWidth<600?225:83,-9.14),new THREE.Vector3(28.4,0,-9.15));}
...
function blocked(x,z){
 if(x< -3||x>59.8||z< -21.3||z>3)return true;
 const radius=.19;
 return data.colliders.some(c=>{
 // 显隐只改变观察方式，实际建筑与设备仍有实体边界。
...
function safePosition(p){
 ...
 return new THREE.Vector3(4.1,1.65,-16.6);
}
...
function buildMap(){
 ...
 function polygon(points,fill,stroke){const el=document.createElementNS(svgNS,'polygon');el.setAttribute('points',points.map(([x,z])=>[16+x*4.35,96+z*4.35].join(',')).join(' '));el.setAttribute('fill',fill);if(stroke){el.setAttribute('stroke',stroke);el.setAttribute('stroke-width','1.5');}map.append(el);}
 polygon([[0,0],[56.8,0],[56.8,-18.3],[0,-18.3]],'#e3ebf0','#829bad');
 polygon([[.3,-.5],[8.35,-.5],[8.35,-14.7],[.3,-14.7]],'#cddde7','#829bad');
 polygon([[8.35,-.5],[16.4,-.5],[16.4,-14.7],[8.35,-14.7]],'#d5dedc','#829bad');
 for(const r of data.equipment)polygon(r.polygon,'#7893a4');
 const entry=document.createElementNS(svgNS,'path');entry.setAttribute('d','M27.3 16.4H39.1');entry.setAttribute('stroke','#f9fcfd');entry.setAttribute('stroke-width','4');map.append(entry);
}
...
function updateLabels(){
 ...
 for(const p of points){
  ...
  if(state.mode==='walk'&&d>14)show=false;
...
function animate(now){
 ...
 const mx=16+camera.position.x*4.35,my=96+camera.position.z*4.35;
...
async function init(){
 try{
  ...
  orbit=new OrbitControls(camera,renderer.domElement);orbit.target.copy(target);orbit.enableDamping=true;orbit.dampingFactor=.08;orbit.minDistance=5;orbit.maxDistance=280;orbit.maxPolarAngle=Math.PI/2-.035;orbit.minPolarAngle=.015;orbit.autoRotateSpeed=.6;orbit.enablePan=true;orbit.screenSpacePanning=false;
  ...
  const sun=new THREE.DirectionalLight(0xfff4df,2.2);sun.position.set(14,42,30);sun.target.position.set(28.4,0,-9.15);scene.add(sun,sun.target);sun.castShadow=true;sun.shadow.mapSize.set(touch?1024:2048,touch?1024:2048);Object.assign(sun.shadow.camera,{left:-40,right:40,top:30,bottom:-30,near:.5,far:120});sun.shadow.bias=-.0003;sun.shadow.normalBias=.025;
  const fill=new THREE.DirectionalLight(0xd7eaff,1.4);fill.position.set(40,7,-13);scene.add(fill);
  const ground=new THREE.Mesh(new THREE.PlaneGeometry(400,400),new THREE.MeshStandardMaterial({color:0xe3ebef,roughness:1}));ground.rotation.x=-Math.PI/2;ground.position.set(28.4,-.17,-9.15);ground.receiveShadow=true;scene.add(ground);
  const grid=new THREE.GridHelper(250,100,0xc9d9e1,0xd8e4eb);grid.position.set(28.4,-.165,-9.15);grid.material.transparent=true;grid.material.opacity=.42;scene.add(grid);
  ...
  const gltf=await loader.loadAsync('./yaxin.glb',e=>{const n=e.total?Math.min(94,Math.round(e.loaded/e.total*94)):Math.min(90,10+e.loaded/50000);$('load-progress').style.width=n+'%';$('load-text').textContent='正在加载模型 '+(e.total?Math.round(e.loaded/e.total*100)+'%':(e.loaded/1048576).toFixed(1)+' MB');});
  ...
  // 屋顶不遮挡室内补光；固定设施、设备仍投射阴影。
  layers.roof?.traverse(o=>{if(o.isMesh)o.castShadow=false;});
  $('rack-count').textContent=data.equipment_count+' 台纺织设备';
  $('destination').innerHTML='<option value="">选择区域，进入漫游</option>'+points.map(p=>'<option value="'+p.id+'">'+p.title+'</option>').join('');buildMap();buildLabels();setupInput();
  ...
  // 诊断接口用于检查相机、图层和通道连通性。
  window.walkthroughDemo={state,camera,scene,renderer,layers,data,blocked,goTo,setMode,setLayer,points};
 }catch(e){loadError(e);}
}
init();
